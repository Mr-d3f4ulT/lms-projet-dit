...
var InstructorNameaddCourse = document.getElementById(
  "InstructorNameaddCourse",
);
...
AddCourseBtn.addEventListener("click", () => {
  ...
  for (var i = 0; i < usersFromStorage[indexUser].Courses.length; i++) {
    if (usersFromStorage[indexUser].Courses[i].CourseName == selectedValue)
      CourseFound = true;
  }
...
function searchByInstructor() {
  ...
  filteredCourses.forEach((course, index) => {
    AddNewCourseToBox(course.CourseName, course.CourseInstructor, index);
  });
}
